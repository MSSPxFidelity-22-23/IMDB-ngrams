...
lemme <- token %>% mutate(lemmetization = lemmatize_words(word)) %>%
  count(lemmetization, sort = TRUE)
#3782 obs.
...
stemming <- token %>%
  mutate(`Remove S` = str_remove(word, "s$"),
         ...
         `lemmetization` = lemmatize_words(word)) %>%
  rename(`Original word` = word)

stemming %>%
  gather(Type, Result, `Remove S`:`lemmetization`) %>%
